...
class Reservation(models.Model):
    ...
    def clean(self):
        ...
        opening_time = datetime.combine(self.date, datetime.min.time()).replace(hour=8, minute=0, second=0,
                                                                                microsecond=0)
        closing_time = datetime.combine(self.date, datetime.min.time()).replace(hour=20, minute=0, second=0,
                                                                                microsecond=0)
